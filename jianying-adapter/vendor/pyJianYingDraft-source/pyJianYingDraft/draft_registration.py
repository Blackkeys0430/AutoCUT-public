...
class DraftFolderRegistration:
    ...
    def __init__(
        ...
        *,
        ...
        registration_root_path: Optional[str] = None,
    ) -> None:
        self.draft_root_path = os.path.abspath(draft_root_path)
        self.user_data_path = user_data_path
        self.content_codec = content_codec
        # IO can use the physical profile while Jianying keeps its host alias
        # in metadata. Never permit an alias for a different directory.
        self.registration_root_path = os.path.abspath(registration_root_path or draft_root_path)
        self._explicit_registration_root = registration_root_path is not None
        if self._explicit_registration_root and not os.path.samefile(
            self.draft_root_path, self.registration_root_path
        ):
            raise ValueError("registration root must reference the same physical draft root")

    def registration_path(self, draft_path: str) -> str:
        if not self._explicit_registration_root:
            return draft_path
        relative = os.path.relpath(os.path.abspath(draft_path), self.draft_root_path)
        if relative == os.pardir or relative.startswith(os.pardir + os.sep):
            raise ValueError("draft path escaped the physical draft root")
        return os.path.join(self.registration_root_path, relative)
    ...
    def ensure_draft_id_is_available_for_path(self, draft_id: str, draft_path: str) -> None:
        ...
        if registered_path and (
            not self.filesystem_paths_equal(registered_path, draft_path)
        ):
            raise ValueError(
    ...
    def sync_draft_meta_info(
        ...
        *,
        ...
        if draft_id:
            ...
            meta_info["draft_root_path"] = self.registration_root_path
            meta_info["draft_fold_path"] = self.registration_path(draft_path)
    ...
    def build_root_meta_entry(
        ...
        *,
        ...
        registered_path = self.registration_path(draft_path)
        managed_fields: Dict[str, Any] = {
            "draft_cover": os.path.join(registered_path, "draft_cover.jpg"),
            "draft_fold_path": registered_path,
            "draft_id": draft_id,
            "draft_json_file": os.path.join(registered_path, "draft_content.json"),
            "draft_name": draft_name,
            "draft_new_version": draft_new_version,
            "draft_root_path": self.registration_root_path,
    ...
    def find_root_meta_entry(
        ...
        for entry in root_entries:
            ...
            if draft_fold_path and self.filesystem_paths_equal(draft_fold_path, draft_path):
                return entry
        ...
        for entry in root_entries:
            if not isinstance(entry, dict) or self.coerce_str(entry.get("draft_id")) != draft_id:
                continue
            draft_fold_path = self.coerce_str(entry.get("draft_fold_path"))
            if not draft_fold_path or self.filesystem_paths_equal(draft_fold_path, draft_path):
                return entry
        return None
    ...
    def collect_root_meta_draft_ids_for_path(
        ...
        return {
            ...
            and self.filesystem_paths_equal(
                self.coerce_str(entry.get("draft_fold_path")), draft_path
            )
    ...
    def upsert_root_meta_entry(
        ...
        for index, entry in enumerate(root_entries):
            ...
            has_same_path = (
                entry_draft_path
                and self.filesystem_paths_equal(entry_draft_path, draft_path)
            )
    ...
    def resolve_draft_id(self, draft_name: str) -> str:
        ...
        for draft_info in self.load_root_meta_info():
            ...
            if not draft_fold_path or not draft_id:
                continue
            if self.filesystem_paths_equal(str(draft_fold_path), expected_draft_path):
                return str(draft_id)
    ...
    def resolve_draft_names_in_current_root(self, draft_ids: List[str]) -> Dict[str, str]:
        ...
        for draft_info in self.load_root_meta_info():
            ...
            if not self.filesystem_paths_equal(os.path.dirname(raw_draft_path), self.draft_root_path):
                continue
    ...
    def root_meta_info_path(self) -> str:
        if self._explicit_registration_root:
            return os.path.join(self.draft_root_path, "root_meta_info.json")
    ...
    @classmethod
    def filesystem_paths_equal(cls, left: str, right: str) -> bool:
        left_path = left.replace("\\", os.sep).replace("/", os.sep)
        right_path = right.replace("\\", os.sep).replace("/", os.sep)
        if cls.normalize_filesystem_path(left) == cls.normalize_filesystem_path(right):
            return True
